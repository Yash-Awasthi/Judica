...
--------------------------------------------------------------------------------
[ ] [P0-01]  Close anonymous /ask cost-exfil â€” Require auth on `/ask` and
             `/ask/stream`, or implement IP-scoped anonymous quota with
             5-req/min cap and direct mode only.

[ ] [P0-02]  Fix user enumeration via timing â€” Always run `argon2.verify`
             against a constant dummy hash when user not found to equalize
             response timing.

[ ] [P0-03]  Hash revokedTokens.token â€” Store `sha256(token)` instead of
             plaintext JWT in the revocation table.

[ ] [P0-04]  Fix logout not revoking cookie-delivered token â€” Extract token
             from cookie on logout, not just Authorization header.

[ ] [P0-05]  Add device/IP binding to /refresh â€” Prevent stolen refresh
             tokens from working on different devices.

[ ] [P0-06]  Fix replay detection â€” On refresh token replay, revoke ALL
             tokens for that user (not just log a warning).

[ ] [P0-07]  Add issuer/audience claims to JWT â€” Pin `issuer: "aibyai"` and
             `audience: env.NODE_ENV` in jwt.sign + jwt.verify.

[ ] [P0-08]  Fix role default("member") â€” Make role required in JWT payload
             schema (no default).

[ ] [P0-09]  Add CSRF protection â€” Either require header-only auth for
             state-mutating routes or implement double-submit CSRF token.

[ ] [P0-10]  Fix authRateLimit multi-replica bypass â€” Move from in-process
             Map to Redis-backed rate limiting.

[ ] [P0-11]  Fix rate-limit IP spoofing â€” Validate X-Forwarded-For against
             trusted proxy list.

[ ] [P0-12]  Fix suspended user can still hit /ask â€” Check `user:status` in
             `fastifyOptionalAuth` too.

[ ] [P0-13]  Fix suspension TTL â€” Remove 30-day EX on Redis suspension key
             (should be permanent until cleared by admin).

### Encryption & Crypto
--------------------------------------------------------------------------------
[ ] [P0-14]  Replace sha256 with proper KDF â€” Use HKDF-SHA256 (for random
             keys) or Argon2id (for passphrases) with stored salt in crypto.ts.

[ ] [P0-15]  Add AAD to AES-256-GCM â€” Bind ciphertext to
             `${tableName}:${recordId}:${fieldName}` to prevent cross-row
             swap attacks.

[ ] [P0-16]  Add version byte to encryption envelope â€” Switch from
             `iv:tag:ct` to `{v:1, iv, tag, ct}` JSON format.

[ ] [P0-17]  Fix encryption detection heuristic â€” Replace
             `apiKey.includes(":")` with proper `isEncrypted()` type-guard
             using JSON envelope.

[ ] [P0-18]  Fix JWT_SECRET min length â€” Change from `min(16)` to `min(32)`
             in env.ts Zod schema.

[ ] [P0-19]  Fix MASTER_ENCRYPTION_KEY validation â€” Require hex-encoded
             32-byte key: `.regex(/^[0-9a-f]{64}$/i)`.

[ ] [P0-20]  Fix key rotation endpoint â€” Don't transmit master keys via
             HTTP body; require env-var + redeploy or KMS.

[ ] [P0-21]  Fix "rotate" endpoint â€” Actually support multiple key versions
             (not just re-encrypt with same key).

### SSRF & Network Security
--------------------------------------------------------------------------------
[ ] [P0-22]  Fix Ollama SSRF bypass â€” Parse URL and check
             `hostname === "localhost" | "127.0.0.1" | "::1"`, not string
             prefix match.

[ ] [P0-23]  Add DNS rebinding protection to ssrf.ts â€” Pin validated IP and
             fetch by IP with Host header preserved.

[ ] [P0-24]  Block 100.64.0.0/10 (CGN) â€” Add to SSRF blocklist (covers
             Alibaba metadata 100.100.100.200).

[ ] [P0-25]  Block TEST-NET ranges â€” Add 192.0.2.0/24, 198.51.100.0/24,
             203.0.113.0/24, 198.18.0.0/15 to blocklist.

[ ] [P0-26]  Add port whitelist to SSRF â€” Only allow 80/443/8080/8443.

[ ] [P0-27]  Add DNS lookup timeout â€” Race against
             `AbortSignal.timeout(2000)`.

[ ] [P0-28]  Redact resolved IP from SSRF error messages â€” Don't leak
             internal network topology.

[ ] [P0-29]  Add `redirect: "error"` to all adapter fetches â€” Prevent
             open-redirect SSRF (strategies had it, adapters lost it).

### Sandbox Security
--------------------------------------------------------------------------------
[ ] [P0-30]  Fix Python sandbox _BLOCKED_MODULES bypass â€” Move preamble to
             separate module not in user namespace; make frozen.

[ ] [P0-31]  Fix _restricted_import reassignment â€” Make
             `__builtins__.__import__` non-writable or use closure-captured
             reference.

[ ] [P0-32]  Replace hand-rolled seccomp BPF with libseccomp â€” Fixes
             x86_64-only numbers, arch check, default posture all at once.

[ ] [P0-33]  Add seccomp_data.arch check â€” Block i386/x32 ABI bypass at
             filter entry.

[ ] [P0-34]  Change SECCOMP_RET_ERRNO to SECCOMP_RET_KILL_PROCESS â€” Comment
             says "kills" but code returns EPERM.

[ ] [P0-35]  Block socket() syscall in seccomp â€” Python-level monkey-patch
             has no backup at kernel level.

[ ] [P0-36]  Block execve/execveat in seccomp â€” Prevent shelling out from
             Python sandbox.

[ ] [P0-37]  Add clone3, process_vm_readv/writev to seccomp blocklist â€”
             Defense-in-depth.

[ ] [P0-38]  Fix socket.fromfd/socketpair not patched â€” Close network escape
             via inherited file descriptors.

[ ] [P0-39]  Fix bash -c shell interpolation â€” Use `execFile` with proper
             arg escaping or avoid `bash -c` entirely.

[ ] [P0-40]  Set killSignal: "SIGKILL" on spawn timeout â€” SIGTERM may not
             kill uninterruptible processes.

### Quota & Cost Control
--------------------------------------------------------------------------------
[ ] [P0-41]  Fix quota TOCTOU â€” Single atomic SQL
             `INSERT ... ON CONFLICT DO UPDATE RETURNING` with rejection
             in same statement.

[ ] [P0-42]  Fix quota increments before request success â€” Only bill after
             successful completion.

[ ] [P0-43]  Fix anonymous usage invisible in metrics â€” Track/log anonymous
             requests even without billing.

[ ] [P0-44]  Remove duplicate fastifyCheckQuota in ask.ts â€” Use only the
             middleware version.

[ ] [P0-45]  Fix preferred-provider bypasses RPM/quota â€” Apply same
             canUse()/checkRPM() checks to preferred provider path.

### Monitoring
--------------------------------------------------------------------------------
[ ] [P0-46]  Fix Prometheus high-cardinality label â€” Use
...
### Provider Layer
--------------------------------------------------------------------------------
[ ] [P1-01]  Fix Anthropic adapter parallel tool_use clobber â€” Change scalar
             currentToolId/Name/Args to a Map keyed by tool call index.

[ ] [P1-02]  Fix image_url silently degraded to text â€” Implement proper
             Anthropic image/Gemini fileData support.

[ ] [P1-03]  Fix Ollama adapter missing tool call support â€” Send tools in
             body and parse message.tool_calls.

[ ] [P1-04]  Fix strategy streaming drops tools â€” Anthropic/Google strategies
             don't send tools in stream mode.

[ ] [P1-05]  Fix API key in URL (Google strategy) â€” Move to
             `x-goog-api-key` header.

[ ] [P1-06]  Fix custom adapter caches decrypted key forever â€” Clear after
             use or use secret-manager abstraction.

[ ] [P1-07]  Fix Anthropic `anthropic-version: 2023-06-01` â€” Update to
             current version for parallel tools, cache_control.

[ ] [P1-08]  Fix OpenRouter adapter missing
             transforms/route/provider.order â€” Use OpenRouter's built-in
             routing features.

[ ] [P1-09]  Extract OpenAICompatibleAdapter base class â€” Deduplicate
             OpenAI/Groq/OpenRouter (95% identical code).

[ ] [P1-10]  Fix SSE parser line-splitting â€” Split on `\n\n` boundary, not
             single `\n`.

[ ] [P1-11]  Fix breaker.fire() cast to Response â€” Add runtime guard for
             open-breaker TypeError.

[ ] [P1-12]  Fix abort signal not propagated to recursive tool calls â€” All
             three strategies drop AbortSignal in recursion.

### Auth & Session
--------------------------------------------------------------------------------
[ ] [P1-13]  Fix cookie-based auth without CSRF on admin routes â€” Admin
             browser can be forced to delete users via CSRF.

[ ] [P1-14]  Fix custom_instructions persistent prompt injection â€” Sanitize
             or length-cap custom instructions field.

[ ] [P1-15]  Fix PUT /settings accepts arbitrary JSON â€” Add schema validation
             with key whitelist.

[ ] [P1-16]  Fix role hierarchy confusion â€” Unify "owner/admin/member/viewer"
             across all endpoints.

[ ] [P1-17]  Prevent admin self-demotion to zero-admin state â€” Require at
             least one admin remains after any role change.

[ ] [P1-18]  Fix hard-delete users without cascade â€” Add soft-delete or
             proper cascade on related tables.

### Rate Limiting
--------------------------------------------------------------------------------
[ ] [P1-19]  Wire rate-limit Redis into @fastify/rate-limit â€” Currently Redis
             client constructed but never attached.

[ ] [P1-20]  Fix silent fallback to in-memory on Redis failure â€” Add /ready
             probe, health gate to prevent degraded-mode startup.

[ ] [P1-21]  Fix sandbox rate-limit multi-replica bypass â€” Move to
             Redis-backed limiting.

[ ] [P1-22]  Add sandbox concurrency cap â€” 10 req/min Ã— 10s = 100s
             CPU/user/min with no ceiling on concurrent executions.

### Middleware
--------------------------------------------------------------------------------
[ ] [P1-23]  Unify token revocation checks â€” One `isTokenRevoked` used by
             both optional and required middleware.

[ ] [P1-24]  Fix providerSchema.baseUrl allows file:// gopher:// â€” Restrict
             to http/https at schema layer.

[ ] [P1-25]  Fix askSchema.context 100KB Ã— enrichment = 500KB+ prompts â€”
             Add post-enrichment size cap.

[ ] [P1-26]  Fix CSP script-src allows cdn.jsdelivr.net â€” Pin to specific
             package subpaths or use SRI hashes.

[ ] [P1-27]  Fix CSP connect-src ws: wss: without origin restriction â€”
             Specify your domain.

[ ] [P1-28]  Fix request ID trusted blindly from header â€” Validate format or
             always generate server-side.

### Config & Env
--------------------------------------------------------------------------------
[ ] [P1-29]  Fix ENABLE_VECTOR_CACHE only accepts "true"/"1" â€” Use proper
             boolean coercion in Zod schema.

[ ] [P1-30]  Fix PORT as string not number â€” Use
             `z.coerce.number().int().positive()`.

[ ] [P1-31]  Fix TRUST_PROXY accepts any string â€” Validate as
             number/boolean/CIDR.

[ ] [P1-32]  Add missing env vars to Zod schema â€” OTEL_EXPORTER_OTLP_ENDPOINT,
             SENTRY_DSN, SMTP_*, GRACEFUL_SHUTDOWN_MS, LANGFUSE_BASEURL.

[ ] [P1-33]  Fix process.exit(1) in env module â€” Throw instead (kills test
             runners, breaks vitest).

[ ] [P1-34]  Add unknown-key warnings for env vars â€” Catch typos like
             JWT_SECERT at startup.

### Sandbox
--------------------------------------------------------------------------------
[ ] [P1-35]  Fix JS sandbox output unbounded â€” Cap output array to prevent
             Node heap exhaustion.

[ ] [P1-36]  Add "use strict" to JS sandbox â€” Prepend to user-wrapped code.

[ ] [P1-37]  Fix seccomp policy regenerated per invocation â€” Cache static
             policy at startup.

[ ] [P1-38]  Fix isSeccompAvailable false positive â€” Check
...
--------------------------------------------------------------------------------
[ ] [P2-01]  Pick src/adapters/ as canonical layer â€” It has the superior
             type system; all other layers should delegate to it.

[ ] [P2-02]  Delete src/lib/providers/* + lib/providers.ts â€” Migrate all
             callers to adapter types.

[ ] [P2-03]  Replace src/lib/providerRegistry.ts with Zod schema over
             config/providers.json â€” Expose read-only metadata from adapter
             registry.

[ ] [P2-04]  Delete src/config/providerConfig.ts â€” Fourth abstraction; merge
             into adapter registry.

[ ] [P2-05]  Delete src/lib/strategies/* (3 files) â€” Strictly inferior to
             adapters; have regressions (no tool streaming, key in URL).

[ ] [P2-06]  Unify type shapes â€” Two Usage types (snake_case vs camelCase),
             two Message types, three fallback systems.

### Fallback & Router
--------------------------------------------------------------------------------
[ ] [P2-07]  Fix config/fallbacks.ts always-Gemini â€” Doesn't implement
             README's chain; either delete or make it emergency-last-resort only.

[ ] [P2-08]  Fix fallback silently disabled if GOOGLE_API_KEY empty â€” Add
             startup warning.

[ ] [P2-09]  Fix config/fallbacks.ts contradicts providers.json fallbacks â€”
             Single source of truth needed.

[ ] [P2-10]  Fix providerChain: gemini appears in both FREE and PAID chains
             â€” find() returns wrong entry.

[ ] [P2-11]  Fix selectProvider estimatedTokens dead parameter â€” Either wire
             it in or remove.

[ ] [P2-12]  Fix routeAndCollect returns "auto" as provider name â€” Metrics
             and audit logs lying about actual provider used.

[ ] [P2-13]  Fix recordRequest fires before request succeeds â€” Move to
             .then() after adapter.generate().

[ ] [P2-14]  Fix RPM/quota TOCTOU race â€” Atomic check-and-reserve or
             document as known limitation.

[ ] [P2-15]  Add circuit breaker to adapter path â€” Only legacy baseProvider
             has Opossum; modern adapters have none.

### Naming & Organization
--------------------------------------------------------------------------------
[ ] [P2-16]  Fix service naming inconsistency â€” Rename 5 `camelCaseService.ts`
             files to match 42 `dot.service.ts` files.

[ ] [P2-17]  Rename src/router/ or src/routes/ â€” Naming collision (provider
             routing vs HTTP routes).

[ ] [P2-18]  Merge lib/validation.ts and lib/validator.ts â€” Duplicate
             validation files.

[ ] [P2-19]  Merge lib/metrics.ts and lib/prometheusMetrics.ts â€” Duplicate
             metrics implementations.

[ ] [P2-20]  Fix lib/cache.ts flat file alongside lib/cache/ directory â€”
             Delete the original file.

[ ] [P2-21]  Merge lib/db.ts and lib/drizzle.ts â€” Two DB entrypoints,
             unclear which is canonical.

[ ] [P2-22]  Consolidate cost/usage (5 places) â€” lib/cost.ts,
             lib/realtimeCost.ts, services/usageService.ts, routes/costs.ts,
             routes/usage.ts.

[ ] [P2-23]  Fix council logic scattered across 7+ files in 2 directories â€”
             Risk of circular imports.

### Redis
--------------------------------------------------------------------------------
[ ] [P2-24]  Drop `redis` package, standardize on ioredis â€” Two Redis
             clients = double pools, double auth, double failure surface.

### Database & Schema
--------------------------------------------------------------------------------
[ ] [P2-25]  Fix providers.json stale model names â€” claude-3-opus without
             date suffix 400s; Groq's llama-3.1-70b retired.

[ ] [P2-26]  Delete LM Studio from providers.json â€” No adapter exists
             (ghost provider silently fails).

[ ] [P2-27]  Remove or gate RPA providers â€” Browser automation against
             SaaS AI = ToS violation risk.

[ ] [P2-28]  Fix quotas.ts â€” Two hardcoded constants; needs tiers,
             env-override, per-route support.

[ ] [P2-29]  Add composite indexes â€” On (user_id, created_at) for
             marketplace/annotations/voting tables.

[ ] [P2-30]  Fix council_configs encryption contradiction â€” PUT writes
...
### Adapter Bugs
--------------------------------------------------------------------------------
[ ] [P3-01]  Fix Groq adapter multi-modal JSON.stringify â€” Flattens image
             blocks to garbage.

[ ] [P3-02]  Fix OpenRouter adapter no multi-modal forwarding â€” Same
             JSON.stringify image issue.

[ ] [P3-03]  Fix Anthropic usage split across two chunks â€” Accumulate inside
             adapter, emit once at end.

[ ] [P3-04]  Fix adapter import of legacy Provider type â€” Decouple from the
             layer being deleted.

[ ] [P3-05]  Fix OpenAI adapter model filter hardcoded â€” Won't pick up gpt-5;
             excludes embedding/tts models.

[ ] [P3-06]  Fix Anthropic listModels() hardcoded array â€” Use real
             /v1/models endpoint.

[ ] [P3-07]  Fix Ollama adapter 120s timeout hardcoded â€” Make
             env-configurable.

[ ] [P3-08]  Fix Gemini adapter no safety settings â€” Add configurable
             safetySettings per request.

[ ] [P3-09]  Fix Gemini functionResponse wraps as {content: JSON} â€” Wrong
             semantics for Gemini API.

### Router Bugs
--------------------------------------------------------------------------------
[ ] [P3-10]  Fix Anthropic path only records completion tokens â€” Usage from
             TWO chunks overwrites; must accumulate not overwrite.

[ ] [P3-11]  Fix routeAndCollect stream.collect() + stream iteration
             interaction â€” Second iteration returns empty.

[ ] [P3-12]  Fix chain model preferred-provider branch â€” Doesn't apply
             `selected.model` (undefined sent to adapter).

[ ] [P3-13]  Fix RPM limiter O(n) shift() â€” Use ring buffer or Deque for
             high-RPM providers.

[ ] [P3-14]  Fix RPM limiter no tenant/user scoping â€” One noisy user
             exhausts everyone's RPM budget.

[ ] [P3-15]  Fix quotaTracker keyed by provider only â€” Should be per-user.

[ ] [P3-16]  Fix quotaTracker resetQuota no auth check â€” Document as
             admin-only or add guard.

[ ] [P3-17]  Fix OpenRouter :free model daily limits wrong â€” Claims 200
             req/day in config, actual is ~20.

### Ask Route Bugs
--------------------------------------------------------------------------------
[ ] [P3-18]  Fix effectiveMembers = [] in direct mode â€” Empty array is
             truthy, doesn't trigger defaults.

[ ] [P3-19]  Fix validateAskBody not async â€” Fastify preHandler expects
             async functions.

[ ] [P3-20]  Fix userId ?? 0 fallback â€” Can access user 0's data; use null
             and reject anonymous explicitly.

[ ] [P3-21]  Fix semantic cache cross-tenant data flow â€” Anonymous user
             populates cache, authenticated users read it.

### Admin Route Bugs
--------------------------------------------------------------------------------
[ ] [P3-22]  Fix admin parseInt(id) without NaN check â€” Send 400 on
             non-numeric IDs.

[ ] [P3-23]  Fix admin sortBy SQL injection risk â€” Whitelist allowed sort
             columns.

[ ] [P3-24]  Fix admin audit export OOM â€” Stream response instead of
             buffering 50k rows.

[ ] [P3-25]  Fix admin PATCH /config no key whitelist â€” Validate allowed
             config keys.

[ ] [P3-26]  Fix council PUT /config customArchetypes unbounded â€” Add max
             length + systemPrompt cap per archetype.

### Workflow / Other
--------------------------------------------------------------------------------
[ ] [P3-27]  Fix workflow loop node eval() on user-controlled expression â€”
             Replace eval() with safe expression parser (expr-eval, jsep).
             This is an RCE vector in the main Node.js process.

[ ] [P3-28]  Fix cross-vendor API key leak â€” OpenAI API key sent in
...
### Security Hardening
--------------------------------------------------------------------------------
[ ] [P4-01]  Add CSRF protection plugin â€” @fastify/csrf-protection.

[ ] [P4-02]  Add Helmet CSP/HSTS env knobs â€” Configurable content security
             policy per environment.

[ ] [P4-03]  Add constant-time comparison + HMAC-SHA256 helpers to crypto.ts
             â€” For webhook signature validation.

[ ] [P4-04]  Add Zod validation to all config file loading â€” Replace
             hand-rolled checks in providers.json, archetypes.ts, etc.

[ ] [P4-05]  Add per-route rate limit overrides â€” Sandbox needs 1/min, not
             global 10.

[ ] [P4-06]  Add multi-tenant/org isolation â€” Currently per-user only;
             enterprises need org boundaries.

[ ] [P4-07]  Add threat model document â€” Beyond SECURITY.md, especially for
             sandbox + MCP client attack surfaces.

### Observability
--------------------------------------------------------------------------------
[ ] [P4-08]  Add OTEL_EXPORTER_OTLP_ENDPOINT to env â€” Tracer exists but no
             config knob.

[ ] [P4-09]  Add SENTRY_DSN or error tracking â€” Pino+Prometheus poor for
             exception triage.

[ ] [P4-10]  Add GRACEFUL_SHUTDOWN_MS env â€” BullMQ + Fastify need SIGTERM
             coordination.

[ ] [P4-11]  Add Redis memory cap guidance â€” Will OOM under long-running
             artifact streams.

[ ] [P4-12]  Add per-tenant Prometheus labels â€” Per-tenant SLO tracking
             impossible without it.

[ ] [P4-13]  Add router_exhausted_total Prometheus counter â€” No telemetry
             for "all providers exhausted" case.

### Infrastructure
--------------------------------------------------------------------------------
[ ] [P4-14]  Move artifact stream from EventEmitter to Redis Streams â€”
             Multi-replica support.

[ ] [P4-15]  Add Redis-backed checkpoints for background agents â€” Currently
             in-memory only; lost on restart.

[ ] [P4-16]  Add worker autoscaling signals â€” BullMQ job-lag metrics to
             Prometheus for HPA.

[ ] [P4-17]  Add queue priorities â€” Deliberation vs background research
             shouldn't share a lane.

[ ] [P4-18]  Add per-source timeout for federated search â€” One hanging
             backend stalls entire call.

### Provider & Routing
--------------------------------------------------------------------------------
[ ] [P4-19]  Add more providers â€” Azure OpenAI, Bedrock, Vertex AI, Fireworks,
             Together, DeepInfra.

[ ] [P4-20]  Add vector DB adapter abstraction â€” Let Pinecone/Weaviate/Qdrant
             plug in alongside pgvector.

[ ] [P4-21]  Add embedding model abstraction â€” Required for RAG pipeline
             flexibility.

[ ] [P4-22]  Make provider chain env/config-configurable â€” Currently
             hardcoded; requires code deploy to change chain.

[ ] [P4-23]  Add priority/tag system to router â€” Express "prefer fast over
             cheap" or "prefer tool-capable".

[ ] [P4-24]  Add per-route rate limit differentiation â€” Rather than global
             knob.

[ ] [P4-25]  Add AbortSignal support to route() â€” No cancellation propagation
             currently.

[ ] [P4-26]  Add provider health probes endpoint â€” Circuit breaker state
             visible for dashboards.

### API Enhancements
--------------------------------------------------------------------------------
[ ] [P4-27]  Add consensus explainability API â€” Expose penalty breakdown
             (claimScore, debateScore, diversityBonus).

[ ] [P4-28]  Add per-provider cost ledger endpoint â€” `/api/costs/breakdown`.

[ ] [P4-29]  Add reliability score admin panel API â€” Reset/override after
             model upgrades.

[ ] [P4-30]  Add deliberation replay endpoint â€”
             `GET /api/deliberations/:id/replay`.

[ ] [P4-31]  Add webhook dead-letter inspection API â€” DLQ browser/retry
             endpoint.

[ ] [P4-32]  Add pagination & filter standards â€” Cursor vs offset decisions
             documented and enforced.

[ ] [P4-33]  Add SOC 2 audit log export endpoint â€” Structured export with
             proper auth.

[ ] [P4-34]  Add S3/GCS for artifact storage â€” Current in-memory stream
             won't scale.

### OSS Hygiene
--------------------------------------------------------------------------------
[ ] [P4-35]  Add LICENSE file â€” MIT or Apache-2.0 (non-negotiable for OSS
             credibility).

[ ] [P4-36]  Rename package from council-project to aibyai â€” Branding
             mismatch in package.json.

[ ] [P4-37]  Add keywords to package.json â€” Currently empty; terrible for
             discoverability.

[ ] [P4-38]  Add Prettier/format script â€” No formatter = style drift between
             committers.

[ ] [P4-39]  Add prepare husky hook â€” Commit-time linting currently skipped.

[ ] [P4-40]  Add "packageManager" field to package.json â€” Prevent lockfile
             drift across Node versions.

[ ] [P4-41]  Write Perplexity adapter or delete orphan test â€”
             tests/adapters/perplexity.test.ts has no implementation.

[ ] [P4-42]  Fix README: 5 archetypes vs actual 14 â€” Documentation drift.

[ ] [P4-43]  Fix README: provider table missing NVIDIA, Xiaomi, Cerebras â€”
             Listed in env but not docs.

[ ] [P4-44]  Add SMTP/email env variables â€” HITL approvals and webhooks need
             notification delivery.

[ ] [P4-45]  Add bench script â€” autocannon installed but no wired benchmark.
...
[ ] [P5-01]  Verify TypeScript 6.0.3 compatibility â€” Ahead of stable; may
             have ecosystem breakage.

[ ] [P5-02]  Verify ESLint 10.2.0 compatibility â€” Major ahead of v9.x flat
             config era.

[ ] [P5-03]  Audit pdf-parse 2.4.5 for CVEs â€” Historically notorious for
             security issues.

[ ] [P5-04]  Pin jsonwebtoken 9.0.3 strictly â€” Had algorithm-confusion
             advisory.

[ ] [P5-05]  Replace passport-github2 0.1.12 â€” Effectively abandoned; use
             @fastify/oauth2.

[ ] [P5-06]  Fix Drizzle ORM 0.45.2 â€” Verify vs latest for HNSW index DDL
             support.

[ ] [P5-07]  Fix swagger-jsdoc types-only â€” @types in devDeps but no runtime
             dep; OpenAPI spec may be missing.

[ ] [P5-08]  Remove dompurify override â€” Audit all consumers of the
             vulnerable component.

[ ] [P5-09]  Fix test:ci --bail=1 â€” Hides downstream failures; won't show
             full failure surface.

[ ] [P5-10]  Add topic graph TTL/GC â€” Currently unbounded growth.

[ ] [P5-11]  Fix memoryCrons.ts location â€” Should be in queue/, not lib/.

[ ] [P5-12]  Add tests for sweeper.ts â€” Cron-based sweepers silently break.

[ ] [P5-13]  Fix duplicate archetypes.ts â€” config/archetypes.ts vs
             lib/archetypes.ts.

[ ] [P5-14]  Move archetype prompts to structured format â€” YAML + schema
             validation, not TS string literals.

[ ] [P5-15]  Fix Gemini default model rolling alias â€” Pin explicit version
             instead of gemini-2.5-flash.

[ ] [P5-16]  Fix Mistral model rolling alias â€” Pin mistral-small-2501 not
             mistral-small-latest.

[ ] [P5-17]  Fix tokenEstimator dead code â€” Result unused by selectProvider;
             either wire in or remove.

[ ] [P5-18]  Fix mask() leaks 25% of key â€” Show only last-4, not
             first-4 + last-4.

[ ] [P5-19]  Fix eager env import in crypto.ts â€” Triggers validation
...
### Critical Test Gaps
--------------------------------------------------------------------------------
[ ] [P6-01]  validateSafeUrl is mocked in every test â€” Real SSRF protection
             never exercised. Add integration tests with a real HTTP server
             asserting private IPs are blocked.

[ ] [P6-02]  isolated-vm fully mocked in sandbox tests â€” Actual VM escape
             surface never tested. Add at least one test running real
             isolated-vm with known safe/unsafe code.

[ ] [P6-03]  Diagnostic scripts inside tests/ make real external API calls â€”
...
[ ] [P6-04]  E2E auth.spec.ts tautological assertions â€” Patterns like
             `expect(x || true).toBeTruthy()` always pass. Tests broken auth
             report green. Replace with strict equality on all auth assertions.

[ ] [P6-05]  E2E kb-upload.spec.ts uploads nothing â€” FormData built but no
             file appended. Rewrite with real synthetic file and assert
             retrieval by content.

[ ] [P6-06]  E2E workflow.spec.ts always-pass assertion â€”
             `expect(['completed','running','failed']).toContain(status)`
             passes for any status. Assert `status === 'completed'`.

### High-Priority Test Gaps
--------------------------------------------------------------------------------
[ ] [P6-07]  contradictionResolution service has zero tests â€” Core council
             correctness logic never verified. Add unit tests for detection,
             non-detection, and performance at n=14 archetypes.

[ ] [P6-08]  backgroundAgents tests use 8 real setTimeout calls â€” Flaky and
             slow. Replace with vi.useFakeTimers().

[ ] [P6-09]  hitlGates tests use real timers for timeout scenarios â€” Same
             risk. Use fake timers; add explicit test for expiry â†’ timeout
             branch transition.

[ ] [P6-10]  Webhooks service has no SSRF test coverage â€” No test verifies
             private IP ranges are blocked before delivery.

[ ] [P6-11]  vectorStore safeVectorLiteral injection untested â€” Malformed
             embeddings (NaN, Infinity, wrong dim) should be rejected before
             SQL layer.

[ ] [P6-12]  Workflow executor has no timeout test â€” Infinite-loop workflow
             should be terminated within configured wall-clock budget.

### Medium-Priority Test Gaps
--------------------------------------------------------------------------------
[ ] [P6-13]  SSE streaming tests don't cover split-chunk messages â€” A single
             SSE event spanning two TCP packets is never tested. Real parse
             behavior differs from mocked.

[ ] [P6-14]  Three duplicate provider implementations in test helpers â€”
             Consolidate into shared test fixture.

[ ] [P6-15]  audioVideo service tests only failure paths â€” No happy-path test
             with synthetic audio fixture.

[ ] [P6-16]  RRF scoring formula mathematically unverified â€” Test with known
...
--------------------------------------------------------------------------------
[ ] [P7-01]  Fix side-effect initialization â€” `initBuiltinAdapters()` runs at
             module load; breaks unit tests and makes registry untestable in
             isolation. Make registry lazy via memoized getter.

[ ] [P7-02]  Fix "llama" heuristic collision â€” `"llama"` substring matches
...
[ ] [P7-03]  Fix Mixtral misrouted to Groq only â€” Registry routes all
             "mixtral" models to Groq, ignoring the Mistral adapter entirely.
             Add explicit Mistral routing rule.

[ ] [P7-04]  Add cleanup before re-init â€” No clear/reset before
...
--------------------------------------------------------------------------------
[ ] [P7-05]  Fix tool arguments type â€” `arguments: Record<string, unknown>`
...
--------------------------------------------------------------------------------
[ ] [P7-10]  Fix __dirname fragile under bundling â€” `__dirname` breaks when
             the module is bundled (esbuild/tsup). Use
             `new URL(import.meta.url)` or embed config as a module import.

[ ] [P7-11]  Expand default registry â€” Default registry hardcodes only 3
...
[ ] [P7-12]  Add cache invalidation â€” Registry has no mechanism to
...
--------------------------------------------------------------------------------
[ ] [P7-13]  Fix string-based error matching â€” Error handling uses
             `error.message.includes("rate limit")` style matching. Brittle:
             any provider that changes its error wording silently breaks
             fallback logic.

[ ] [P7-14]  Fix retry Ã— fallback explosion â€” Retry logic and fallback logic
...
[ ] [P7-15]  Fix isFallback manual propagation â€” `isFallback` flag manually
...
--------------------------------------------------------------------------------
[ ] [P7-16]  Fix error strings tightly coupled to callers â€” Factory throws
             raw strings like `"Unknown provider type"` that callers match
             with `error.message.includes(...)`. Use typed error classes.

[ ] [P7-17]  Remove dead branch "openai-compat" â€” Dead dispatch branch in
             factory that was never wired up. Remove to reduce confusion.

### lib/providers/baseProvider.ts
--------------------------------------------------------------------------------
[ ] [P7-18]  Add per-request timeout â€” baseProvider has no request-level
...
--------------------------------------------------------------------------------
[ ] [P7-19]  Type ContentBlock properly â€” `ContentBlock` defined as
             `[key: string]: unknown`. Add discriminated union:
             `{ type: "text", text: string } | { type: "image", ... }`.

[ ] [P7-20]  Fix userId forced as number â€” Legacy type forces `userId:
             number`, incompatible with UUID-based auth. Change to
             `string | number` or migrate to string throughout.

[ ] [P7-21]  Add multi-modal typing to legacy layer â€” Legacy provider types
...
[ ] [P7-26]  Add tool_call_id validation â€” Anthropic tool-use responses lack
...
--------------------------------------------------------------------------------
[ ] [P7-31]  Fix hardcoded referer header â€” `HTTP-Referer` header hardcoded
             to a fixed value. Should be configurable via env var for white-
             label deployments.

### Ollama Adapter â€” Additional Bugs
--------------------------------------------------------------------------------
[ ] [P7-32]  Fix dual timeout system conflict â€” Ollama adapter has both a
...
[ ] [P7-33]  Fix incorrect role mapping â€” Ollama uses `"assistant"` where
             some message chains require `"model"` (Gemini convention leaking
             in); causes API rejection on strict endpoints.

[ ] [P7-34]  Add model availability check â€” Ollama adapter sends requests
...
--------------------------------------------------------------------------------
[ ] [P7-40]  Add recursion depth limit â€” Tool-call recursion in all three
...
[ ] [P7-41]  Standardize SSRF handling across strategies â€” Strategies apply
...
--------------------------------------------------------------------------------
[ ] [P7-42]  Fix SUMMONS list redundant â€” SUMMONS array lists archetypes to
...
[ ] [P7-43]  Fix inconsistent tool assignment â€” Some archetypes have no
             tools array, some have partial sets, some have all tools. No
             policy document defines which archetypes should have which
             capabilities.

[ ] [P7-44]  Unify persona systems â€” Two persona mechanisms coexist:
...
--------------------------------------------------------------------------------
[ ] [P7-45]  Fix blocking console calls â€” `console.log` in sandbox flushes
             synchronously to stdout, blocking the Node event loop during
             high-volume sandbox output. Buffer and flush asynchronously.

[ ] [P7-46]  Add stdout/stderr separation â€” Sandbox currently mixes stdout
...
[ ] [P7-47]  Add global whitelist â€” JS sandbox restricts some globals by
...
--------------------------------------------------------------------------------
[ ] [P8-01]  Fix /metrics endpoint security â€” Three issues in one:
...
[ ] [P8-02]  Fix CORS allows localhost in production â€” CORS origin list
...
[ ] [P8-03]  Remove global 200KB bodyLimit â€” Global body limit breaks file
...
[ ] [P8-04]  Move static file middleware after API routes â€” Static
             middleware mounted at `/` intercepts API errors and returns HTML
             404s instead of JSON. Mount static after all API route
             registrations.

[ ] [P8-05]  Disable Swagger UI in production â€” Swagger/OpenAPI UI currently
...
[ ] [P8-06]  Add WS/SSE connection limits â€” No maximum on concurrent
             WebSocket or SSE connections per user or globally. A single
             user can exhaust server file descriptors by opening thousands
             of streams.

[ ] [P8-07]  Split /live and /ready health endpoints â€” Current health check
...
[ ] [P8-08]  Remove runtime createRequire on package.json â€” Server reads
...
--------------------------------------------------------------------------------
[ ] [P8-09]  Fix uncaughtException/unhandledRejection exits â€” Both handlers
...
[ ] [P8-10]  Wrap buildApp() in try/catch with classification â€” Startup
...
[ ] [P8-11]  Fix SIGTERM/SIGINT double-signal race â€” No guard against
...
[ ] [P8-12]  Prevent duplicate signal handler registration â€” Signal
...
--------------------------------------------------------------------------------
[ ] [P8-13]  Fix rebuttal classification prompt injection â€” Rebuttal type
...
[ ] [P8-14]  Fix "auto" model â€” hidden cost and no user attribution â€” The
...
[ ] [P8-15]  Fix sanitizeForPrompt ineffective â€” The prompt sanitization
...
[ ] [P8-16]  Fix bus.reset() not called on failure â€” `messageBus.reset()`
...
[ ] [P8-17]  Add token budget check before synthesis â€” Orchestrator
...
[ ] [P8-18]  Track "auto"-routed models in reliability system â€” When the
...
[ ] [P8-19]  Persist pendingHumanGates to Redis â€” HITL gate state stored
...
--------------------------------------------------------------------------------
[ ] [P8-20]  Fix O(nÂ²) LLM calls in conflict detection â€” Every archetype
...
[ ] [P8-21]  Fix JSON parsing via regex â€” Conflict detector parses LLM JSON
...
[ ] [P8-22]  Make severity threshold configurable â€” Conflict severity
...
--------------------------------------------------------------------------------
[ ] [P8-23]  Fix non-atomic shared memory updates â€” Shared memory
...
[ ] [P8-24]  Add ownership validation to sharedMemory â€” Functions that
...
[ ] [P8-25]  Bill LLM fact extraction to user â€” LLM calls inside
             `extractFacts()` are not attributed to the user's quota.
             Hidden cost accumulates without limit.

### agents/messageBus.ts
--------------------------------------------------------------------------------
[ ] [P8-26]  Cap allMessages array â€” `allMessages` grows unbounded for the
...
[ ] [P8-27]  Fix subscription race during broadcast â€” Broadcasting to
...
[ ] [P8-28]  Log/error on unknown agent message â€” Messages sent to an
             agent name not in the registry are silently dropped. Add a
             warning log so misconfigured workflows are detectable.

### agents/personas.ts
--------------------------------------------------------------------------------
[ ] [P8-29]  Move personas to database â€” Persona definitions hardcoded in
             source; runtime updates require a deployment. Move to DB table
             with admin CRUD API.

[ ] [P8-30]  Fix schema mismatch between built-in and custom personas â€”
...
--------------------------------------------------------------------------------
[ ] [P8-31]  Fix GitHub OAuth accepts unverified email â€” GitHub strategy
...
[ ] [P8-32]  Fix Google OAuth weak verification check â€” Google strategy
             checks `if (verified === false)` which passes when `verified`
             is `undefined` or any truthy non-false value. Change to
             `if (verified !== true)`.

[ ] [P8-33]  Add OAuth state parameter validation â€” OAuth callback does not
...
[ ] [P8-34]  Replace passwordHash empty string as auth-type flag â€”
...
[ ] [P8-41]  Define Drizzle relations â€” No Drizzle `relations()` defined
             for any table. Without relations, Drizzle query builder cannot
             perform type-safe joins; all cross-table queries use raw SQL.

[ ] [P8-42]  Move large JSON fields to side tables â€” Several tables store
...
[ ] [P8-43]  Fix denormalized counters without triggers â€” Vote counts,
...
[ ] [P8-44]  Enforce or remove org/tenant system â€” `organizations` table
...
--------------------------------------------------------------------------------
[ ] [P8-48]  Fix incorrect token accounting in quota headers â€” Response
...
### errorHandler.ts
--------------------------------------------------------------------------------
[ ] [P8-49]  Sanitize error responses in all environments â€” Error handler
...
[ ] [P8-50]  Fix weak Zod error detection â€” Zod errors detected via
             `error.name === "ZodError"` string check which can be spoofed.
             Use `error instanceof ZodError`.

[ ] [P8-51]  Include request ID in error responses â€” Error responses don't
...
[ ] [P8-52]  Remove dead isOperational flag â€” `isOperational` property on
...
--------------------------------------------------------------------------------
[ ] [P8-53]  Add member count limit â€” No cap on `members` array in ask
...
[ ] [P8-54]  Enforce minimum password length of 12 â€” Current minimum is 6
...
--------------------------------------------------------------------------------
[ ] [P8-58]  Fix missing histogram timer cleanup â€” Request duration
...
--------------------------------------------------------------------------------
[ ] [P8-59]  Fix in-process rate limit Map memory growth â€” The in-process
...
[ ] [P8-60]  Replace bcryptjs with argon2id â€” bcryptjs is the JS
...
[ ] [P8-61]  Fix prototype pollution via arbitrary JSON settings â€” PUT
...
--------------------------------------------------------------------------------
[ ] [P8-62]  Fix SSE error handling broken â€” When the upstream provider
...
[ ] [P8-63]  Fix artifacts generated but discarded â€” Artifacts (code
...
[ ] [P8-64]  Add authorization checks on RAG/file access â€” Routes that
...
[ ] [P8-65]  Fix token usage not tracked for all paths â€” Token usage is
...
[ ] [P8-66]  Cache hits must not bypass quota â€” Semantic cache hits return
...
### lib/audit.ts
--------------------------------------------------------------------------------
[ ] [P9-01]  logRouterDecision stores raw reasoning without sanitization before
             persistence â€” prompt internals and decision chains stored verbatim.

[ ] [P9-02]  Risk threshold gap (50â€“69) â†’ silent anonymization with no log entry
             â€” events in this band disappear entirely.

[ ] [P9-03]  Double PII scan on same content â†’ high CPU overhead â€” scan runs
             twice per audit record.

[ ] [P9-04]  Inconsistent success semantics â€” `undefined` return counted as
             success; only explicit false is failure.

[ ] [P9-05]  Fake token estimation uses hardcoded 0.4/0.6 input/output split â€”
             actual token counts available but ignored.

[ ] [P9-06]  No index on metadata JSONB queries â†’ full-table O(n) scan on every
             audit search.

[ ] [P9-07]  sessionId stored only in metadata JSON, not as a first-class indexed
             column.

### lib/breaker.ts
--------------------------------------------------------------------------------
[ ] [P9-08]  No LRU/TTL eviction for circuit breakers â†’ memory leak when provider
             list is dynamic (custom providers created/deleted).

[ ] [P9-09]  All error types (including 4xx client errors) increment the failure
             counter â†’ false circuit trips on bad requests.

[ ] [P9-10]  Conflicting timeout systems: Opossum's built-in timeout AND external
             AbortController both active â€” whichever fires first leaves the other
             dangling.

[ ] [P9-11]  Unsafe generic cast on breaker.fire() return value â€” no runtime type
             guard.

### lib/cache.ts
--------------------------------------------------------------------------------
[ ] [P9-12]  embeddingLocks Map unbounded â€” burst traffic creates thousands of
             lock entries never cleaned up.

[ ] [P9-13]  Silent degradation when OpenAI embedding key missing â€” semantic cache
             silently disabled with no log warning.

[ ] [P9-14]  Prompt `.toLowerCase()` normalization causes cache collisions between
             semantically different prompts that differ only by case.

[ ] [P9-15]  History serialization format unstable and bloated â€” full message
             objects serialized; schema changes silently corrupt cached entries.

[ ] [P9-16]  Cache cleanup via `Math.random() < 0.01` probabilistic â€” unreliable;
             old entries accumulate under steady load.

[ ] [P9-17]  Semantic similarity threshold (0.15) hardcoded â€” needs to be
             externalized to config/env.

[ ] [P9-18]  Write-order race: PostgreSQL written first, then Redis â€” if Redis
             write fails, subsequent requests bypass Postgres and miss the entry.

[ ] [P9-19]  Dead optional chaining `setSemantic?.()` â€” method always exists;
             optional chain hides a missing implementation.

### lib/cache/CacheBackend.ts
--------------------------------------------------------------------------------
[ ] [P9-20]  `opinions` type duplicated across multiple cache files â€” should be
             defined once and imported.

[ ] [P9-21]  TTL contract undefined across backends â€” some backends treat missing
             TTL as "no expiry", others as error; no interface contract.

### lib/cache/PostgresBackend.ts
--------------------------------------------------------------------------------
[ ] [P9-22]  No enforcement that vector search index (ivfflat/hnsw) exists before
             query â€” falls back to seqscan silently, order-of-magnitude slower.

[ ] [P9-23]  Full-table sort performed before threshold filtering â€” should push
             similarity threshold into WHERE clause to reduce sort cost.

[ ] [P9-24]  `createdAt` reset to NOW() on cache entry update â€” original creation
             time lost, breaks TTL-based expiry logic.

[ ] [P9-25]  Expired rows fetched from DB then filtered in application code â€”
             should filter in SQL with `WHERE expires_at > NOW()`.

[ ] [P9-26]  Silent delete failures â€” `DELETE` errors caught and swallowed with
             no log.

[ ] [P9-27]  Mixed `opinions` field schema â€” stored as plain string in some rows,
             JSON object in others; no migration.

### lib/cache/RedisBackend.ts
--------------------------------------------------------------------------------
[ ] [P9-28]  `ttlMs = 0` treated as "set with no TTL" â†’ unintended infinite
             persistence; should treat 0 as error or require explicit null.

[ ] [P9-29]  No payload size guard â€” arbitrarily large values stored in Redis;
             can exhaust Redis memory.

[ ] [P9-30]  Semantic cache methods not implemented but implicitly expected â€”
             callers assume semantic ops exist; silent no-ops.

### lib/retry.ts
--------------------------------------------------------------------------------
[ ] [P9-31]  AbortError bypasses `shouldRetry` predicate â€” aborted requests retry
             anyway; correct behavior should be: never retry on abort.

[ ] [P9-32]  Retry count semantics ambiguous â€” `maxRetries: 3` unclear if it
             means 3 total attempts or 3 additional retries after first.

[ ] [P9-33]  No global retry budget â€” each layer (provider, strategy, router)
             retries independently; total attempts multiply.

[ ] [P9-34]  Retry events not emitted to metrics system â€” no visibility into retry
             rate per provider.

[ ] [P9-35]  No cancellation check during retry delay â€” AbortSignal fired during
             backoff sleep is not noticed until the next attempt begins.

### lib/redis.ts
--------------------------------------------------------------------------------
[ ] [P9-36]  `flushAll()` exposed as public method without safeguards â€” can be
             called accidentally in production; should require explicit `DANGER_`
             flag or be removed.

[ ] [P9-37]  Reconnect logic unstable after max retries exceeded â€” client enters
             broken state with no recovery path; process restart required.

[ ] [P9-38]  EX vs PX option precedence undefined â€” when both EX and PX passed to
             SET, behavior depends on ioredis internals; document or enforce one.

[ ] [P9-39]  `KEYS` command used (O(n)) â€” blocks Redis event loop on large
             keyspaces; replace with `SCAN` with cursor.

[ ] [P9-40]  Single Redis client for all operations â€” high-throughput paths (rate
             limiting, cache, pub/sub) share one connection; add pooling or
             separate clients per concern.

### lib/db.ts
--------------------------------------------------------------------------------
[ ] [P9-41]  No TLS/SSL enforcement on DB connection â€” connection string accepted
             without SSL requirement; credentials sent in plaintext on
             non-localhost setups.

[ ] [P9-42]  No `application_name` set on connection pool â€” all connections appear
             as anonymous in pg_stat_activity; impossible to distinguish app
             traffic from migrations.

[ ] [P9-43]  Verbose connection-acquire logging on every request â€” pool events
             logged at INFO level creating log noise proportional to QPS.

[ ] [P9-44]  `NaN` connection limit fallback bug â€” `parseInt(env.DB_POOL_SIZE)`
             returns `NaN` if value is non-numeric; `NaN` passed to pool silently
             uses driver default.

### lib/drizzle.ts
--------------------------------------------------------------------------------
[ ] [P9-45]  No lazy initialization â€” Drizzle ORM client instantiated at module
             load; DB failures during startup crash the process before Fastify
             can report health.

[ ] [P9-46]  Eager schema loading causes cold-start delay and circular import risk
             â€” all table definitions imported synchronously.

[ ] [P9-47]  No query logging â€” no way to capture slow queries or debug
             ORM-generated SQL without external pg_log setup.

[ ] [P9-48]  ORM bypass possible via raw pool reference â€” `db.$client` exposes
             the underlying pool; bypasses all ORM-level validation and hooks.

### lib/logger.ts
--------------------------------------------------------------------------------
[ ] [P9-49]  No redaction config â€” API keys, JWT tokens, passwords may appear in
             logs if included in request objects; Pino's `redact` option not
             configured.

[ ] [P9-50]  No log transport â€” logs written to stdout only; no structured export
             to Loki, OTLP, or any aggregator.

[ ] [P9-51]  No trace/span correlation â€” `traceId` and `spanId` not injected into
             log records; cannot correlate logs with OTEL traces.

[ ] [P9-52]  No sampling â€” every request logged at full verbosity; under load this
             creates I/O pressure and cost.

[ ] [P9-53]  Pino pretty-print used outside dev â€” `pino-pretty` is synchronous
             and blocks the event loop; must not be used in staging/production.

### lib/metrics.ts
--------------------------------------------------------------------------------
[ ] [P9-54]  O(nÂ²) sequential similarity calls â€” archetype response similarity
             computed pairwise in a sequential loop; should be parallelized.

[ ] [P9-55]  ML similarity failure crashes the request â€” no fallback if the
             embedding/similarity call throws; entire deliberation fails.

[ ] [P9-56]  Weak fallback similarity â€” fallback uses token overlap which is
             unreliable for semantic comparison.

[ ] [P9-57]  Consensus threshold hardcoded â€” threshold for "consensus reached" is
             a magic number in source; should be configurable.

[ ] [P9-58]  Empty archetype responses treated as full consensus â€” empty string
             responses score high similarity against each other.

[ ] [P9-59]  Scale mismatch â€” token-count similarity and embedding cosine
             similarity mixed without normalization in final score.

[ ] [P9-60]  No caching of similarity results â€” identical pairs recomputed on
             every deliberation round.

### lib/prometheusMetrics.ts
--------------------------------------------------------------------------------
[ ] [P9-61]  Poor histogram bucket sizing for long-running tasks â€” default buckets
             (0.005 to 10s) miss workflow durations of minutes; add [30, 60, 120,
             300] buckets.

[ ] [P9-62]  Global Prometheus registry â€” using default global registry means test
             runs and multiple app instances share metric state; use a
             per-instance registry.

[ ] [P9-63]  Gauges may never be updated â€” some gauge metrics set at startup but
             never updated by collectors; stale values mislead dashboards.

[ ] [P9-64]  Missing `backend` label on cache metrics â€” cache hit/miss metrics
             don't distinguish Redis vs Postgres backend; impossible to diagnose
             which is slow.

### lib/cost.ts
--------------------------------------------------------------------------------
[ ] [P9-65]  Static pricing table hardcoded in source â€” model prices change
             frequently; requires code deployment to update; externalize to config
             file or pricing service.

[ ] [P9-66]  Token estimator uses static pricing, ignores actual billed tokens â€”
             estimates diverge from reality over time.

[ ] [P9-67]  Cost not persisted to DB â€” computed costs exist only in-memory; lost
             on restart; no historical cost ledger.

[ ] [P9-68]  `byProvider` and `byModel` aggregation fields populated but never
             read â€” dead code inflating object size.

[ ] [P9-69]  Unknown model cost falls back silently to zero â€” unrecognized models
             billed at $0; usage goes unaccounted.

[ ] [P9-70]  Broken efficiency metric â€” efficiency formula produces values outside
             [0,1] for some input combinations.

[ ] [P9-71]  Float precision errors in cost accumulation â€” JavaScript floating
             point causes penny-level drift over many requests; use integer
             microcents.

[ ] [P9-72]  Cost aggregation loses provider/model attribution â€” rolled-up totals
             discard which provider/model incurred the cost.

### lib/realtimeCost.ts
--------------------------------------------------------------------------------
[ ] [P9-73]  Entirely in-memory â€” broken in any multi-replica deployment; each
             replica tracks its own users independently.

[ ] [P9-74]  No daily/monthly reset â€” cost counters grow forever; no time-window
             semantics.

[ ] [P9-75]  State lost on restart â€” all real-time cost data lost on process
             restart or crash.

[ ] [P9-76]  Active user sessions evicted by LRU â€” users mid-session can have
             their cost counter evicted, causing under-billing.

[ ] [P9-77]  Session double-count bug â€” same session counted twice if session-start
             event fires during reconnect.

[ ] [P9-78]  No user-session ownership validation â€” any code can increment any
             user's cost counter by passing an arbitrary userId.

[ ] [P9-79]  Memory leaks from stale callbacks and expired sessions not fully
             cleaned up.

[ ] [P9-80]  Stats aggregate only current-session data â€” lifetime totals not
             tracked; impossible to compute monthly spend.

[ ] [P9-81]  Cost aggregation interval runs globally with no lifecycle control â€”
             interval not cleared on app shutdown; causes test pollution.

### lib/errorMapper.ts
--------------------------------------------------------------------------------
[ ] [P9-82]  Mixed `code` field semantics â€” `code` used for both HTTP status code
             and error type string in different places; consumers must guess which.

[ ] [P9-83]  Misleading status mappings â€” some provider errors mapped to 403
             Forbidden or 402 Payment Required incorrectly.

[ ] [P9-84]  No retry classification â€” error mapper doesn't indicate whether the
             error is retryable; callers duplicate this logic.

[ ] [P9-85]  Structured error data dropped â€” provider errors often include helpful
             structured fields (retry-after, rate limit reset); mapper discards
             them.

### lib/context.ts
--------------------------------------------------------------------------------
[ ] [P9-86]  Only `requestId` in AsyncLocalStorage context â€” `userId`, `traceId`,
             `tenantId` not carried; downstream code must re-fetch from request
             object.

[ ] [P9-87]  No helper for context-wrapped execution â€” consumers must manually
             call `store.run()`; error-prone boilerplate.

[ ] [P9-88]  Context object mutable â€” any code in the call stack can modify the
             context; side effects are invisible to callers.

### lib/history.ts
--------------------------------------------------------------------------------
[ ] [P9-89]  Fake summarization via string truncation â€” "summaries" are just the
             first N characters of the last message; no semantic compression.

[ ] [P9-90]  Keyword-based message retrieval â€” relevant history retrieved by
             keyword match, not semantic similarity; misses paraphrased context.

[ ] [P9-91]  Full table scan for conversation history â€” no limit/offset on history
             fetch for long conversations; O(n) query.

[ ] [P9-92]  Duplicate summaries accumulate â€” summary generation not idempotent;
             multiple summaries created for the same window on concurrent
             requests.

[ ] [P9-93]  History window is fixed message count, not token-aware â€” 20-message
             window can be 100 or 10,000 tokens; no budget enforcement.

[ ] [P9-94]  Weak keyword extraction â€” keywords extracted by splitting on spaces;
             misses stemming, stopwords, multi-word phrases.

[ ] [P9-95]  Wrong role for injected summaries â€” summary injected as `user` role
             message instead of `system`; confuses model context.

[ ] [P9-96]  No validation of stored JSON message structure â€” malformed messages
             stored in DB pass through without error until model call fails.

[ ] [P9-97]  Potential cross-tenant history leak â€” history fetch query filters by
             conversationId only; no userId ownership check.

### lib/tracer.ts
--------------------------------------------------------------------------------
[ ] [P9-98]  No span hierarchy â€” all tracing steps recorded as flat top-level
             spans; parent-child relationships lost; waterfall view impossible.

[ ] [P9-99]  Blocking synchronous DB writes on request path â€” trace persistence
             happens synchronously during request; adds latency to every traced
             call.

[ ] [P9-100] Langfuse client reinitialized per request â€” `new LangfuseClient()`
             called on every trace start; should be a module-level singleton.

[ ] [P9-101] Silent export failure â€” if Langfuse/OTLP export fails, error is
             caught and silently dropped; no alert, no fallback.

[ ] [P9-102] No per-token granularity â€” input vs output tokens not tracked
             separately in spans; only total token count recorded.

[ ] [P9-103] Zero-latency default masks missing instrumentation â€” uninstrumented
             steps default to 0ms latency; dashboards appear healthy when
             instrumentation is broken.

[ ] [P9-104] Unsafe type casting in span attribute assignment â€” span attributes
             cast with `as any`; type errors silently lost.

[ ] [P9-105] Fake cost in traces â€” tracer computes its own cost estimate
...
### lib/adversarial.ts
--------------------------------------------------------------------------------
[ ] [P10-01] Silent failure returns is_robust=true on parse error â€” when the
...
[ ] [P10-02] Greedy JSON regex extraction â€” response parsed with a regex that
...
[ ] [P10-03] No shape validation of parsed output â€” parsed object accepted
...
[ ] [P10-10] ID collision via Date.now() â€” custom archetype IDs generated
             as Date.now() strings; two concurrent imports within the same
             millisecond produce identical IDs. Use UUIDs.

[ ] [P10-11] Hardcoded tool allowlist per archetype â€” which tools each
...
[ ] [P10-12] No transaction on bulk import â€” archetype import inserts rows
...
--------------------------------------------------------------------------------
[ ] [P10-13] Ollama always injected into provider list â€” configResolver
...
### lib/controller.ts
--------------------------------------------------------------------------------
[ ] [P10-20] "Discard" round not actually reverted â€” when a deliberation
...
[ ] [P10-26] Cost always 0 in council response â€” council assembles cost
...
[ ] [P10-30] Non-integer round index â€” round numbers computed via division
...
[ ] [P10-31] Direct access to private controller state â€” council.ts accesses
...
[ ] [P10-32] Wrong consensus signal for skip logic â€” skip condition checks
...
[ ] [P10-33] Prompt mutation across calls â€” system prompt object mutated
             in-place between archetype calls; later archetypes receive
             partially-modified prompts from earlier ones. Clone before
             each mutation.

[ ] [P10-34] No AbortSignal check between rounds â€” long-running councils
...
--------------------------------------------------------------------------------
[ ] [P10-35] Fake outputs injected on JSON parse failure â€” when LLM returns
...
[ ] [P10-36] Schema validation bypassed â€” schema is imported and defined but
             the parse result is never passed through it before use; invalid
             shapes proceed silently.

[ ] [P10-37] Validation function result ignored â€” `validateResponse()` is
             called but its return value discarded; invalid responses
             continue through the pipeline regardless.

[ ] [P10-38] Peer review fallback is garbage data â€” on peer review LLM
...
[ ] [P10-54] Single datapoint bias â€” rolling averages computed on single
             data points produce unstable values; require minimum sample
             size before emitting a metric.

[ ] [P10-55] Silent DB failure â€” evaluation metrics persistence failure
             caught and silently dropped; metrics are lost without any alert.

### lib/grounding.ts
--------------------------------------------------------------------------------
[ ] [P10-56] Silent success on grounding failure â€” when the grounding LLM
             call fails or returns unparseable output, is_grounded defaults
             to true. A failed grounding check should fail closed
             (is_grounded=false).

[ ] [P10-57] Penalizes novel correct answers â€” grounding logic penalizes
...
[ ] [P10-58] Greedy JSON parsing â€” same greedy regex issue as adversarial.ts;
             malformed LLM JSON responses silently corrupt grounding scores.

[ ] [P10-59] No result caching â€” grounding check re-runs for every archetype
             response against every source document; quadratic cost for large
             source sets. Cache by (response_hash, source_hash).

[ ] [P10-60] Weak output validation â€” grounding check result accepted
             without validating required fields; missing confidence field
             silently becomes undefined.

### lib/memoryCrons.ts
--------------------------------------------------------------------------------
[ ] [P10-61] No distributed locking â€” memory compaction cron runs on every
...
--------------------------------------------------------------------------------
[ ] [P10-72] Ignores provider config, always uses "auto" â€” reasoning mode
...
[ ] [P10-87] Broken retry logic â€” retry on node failure re-runs the node
...
[ ] [P10-88] No timeout on HITL gates â€” a workflow waiting for human
             approval blocks indefinitely with no timeout. Add configurable
             expiry; route to timeout-handler branch on expiry.

[ ] [P10-89] Mutates workflow definition object â€” executor modifies the
...
--------------------------------------------------------------------------------
[ ] [P10-103] Prompt injection via template interpolation â€” upstream node
             outputs interpolated directly into LLM prompt without escaping;
             a malicious node output can hijack the prompt.

[ ] [P10-104] Weak variable substitution â€” template engine uses simple
...
[ ] [P10-105] No variable validation â€” template variables not validated
             against declared input schema before substitution; type
             mismatches produce malformed prompts.

[ ] [P10-106] No LLM cost tracking â€” llm node calls not attributed to
             workflow owner's quota; cost invisible in usage reports.

### workflow/nodes/loop.handler.ts
--------------------------------------------------------------------------------
[ ] [P10-107] Heavy isolate creation per iteration â€” a new isolated-vm
...
[ ] [P10-118] No timeout on tool execution â€” tool calls have no timeout;
             a slow or hung tool blocks the workflow indefinitely.

[ ] [P10-119] No output size limit on tool results â€” tool results stored
             in workflow state without size cap; large tool outputs exhaust
             state storage.

[ ] [P10-120] Tool execution not sandboxed at handler level â€” tool handler
...
[ ] [P10-121] No audit log of tool invocations â€” tool name, inputs, and
...
--------------------------------------------------------------------------------
[ ] [P10-122] No conditionals, loops, or filters â€” template engine is
             basic string interpolation only; complex templates require
             workarounds via LLM nodes. Replace with Handlebars or Liquid.

[ ] [P10-123] Cannot escape special characters â€” no escape mechanism for
             template delimiters; data containing {{ }} breaks rendering
             silently.

[ ] [P10-124] No validation for malformed templates â€” templates with
...
--------------------------------------------------------------------------------
[ ] [P10-125] Same LLM for generation, validation, and adversarial â€” no
...
[ ] [P10-130] No deterministic reproducibility â€” no seed or determinism
...
--------------------------------------------------------------------------------
[ ] [P10-136] Multiple injection surfaces not centrally defended â€” prompt
             injection, template injection, and tool input override each
             defended ad-hoc; a central input sanitization layer is absent.

[ ] [P10-137] SSRF TOCTOU still exploitable via redirect chains â€” SSRF
...
[ ] [P10-138] Prototype pollution in merge/split propagates through state â€”
             polluted state object passed to downstream nodes; all nodes
             in a workflow share the pollution.

[ ] [P10-139] Sandbox escape via chained node execution â€” isolated-vm
...
--------------------------------------------------------------------------------
[ ] [P11-01] Tool-call JSON parse failures silently swallowed in tests â€”
