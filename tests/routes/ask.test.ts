...
describe("POST / ask", () => {
  ...
  it("loads RAG context when kb_id and userId are present", async () => {
    ...
    expect(mockLoadRAGContext).toHaveBeenCalledWith(1, "Search KB", "kb-123", undefined, { from: undefined, to: undefined });
...
describe("POST /stream", () => {
  ...
  it("loads file and RAG context in stream mode", async () => {
    ...
    expect(mockLoadRAGContext).toHaveBeenCalledWith(1, "Stream with context", "kb-1", undefined, { from: undefined, to: undefined });
  });
