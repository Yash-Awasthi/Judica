...
describe("runResearch", () => {
  ...
  describe("search fallback", () => {
    ...
    it("sends SerpAPI key via URL query string", async () => {
      (env as any).TAVILY_API_KEY = "";
      ...
      // URL should contain the API key as a query parameter
      expect(String(serpCall![0])).toContain("api_key=serp-key");
      // No separate headers object with the key expected
    });
  });
