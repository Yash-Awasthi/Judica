...
export default function ConnectorsOnboardingPage() {
  ...
  const handleOAuth = () => {
    ...
    sessionStorage.setItem("judica_connector_pending", JSON.stringify({ connectorId: selected.id, step: 2 }));
    window.location.href = selected.oauthUrl + `?redirect_to=/connectors/new&source=${selected.id}`;
  };

  // Check if returning from OAuth
  useEffect(() => {
    const pending = sessionStorage.getItem("judica_connector_pending");
    if (pending) {
      try {
      ...
      sessionStorage.removeItem("judica_connector_pending");
    }
  }, []);
          ...
          {step === 1 && selected && (
            <div className="space-y-5">
              ...
              {selected.authType === "oauth" ? (
                <div className="space-y-4">
                  <p className="text-sm">
                    Click below to authorize Judica to access your {selected.label} account.
