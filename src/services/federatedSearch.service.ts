...
import { expandQuery } from "../lib/queryExpansion.js";
...
export interface FederatedSearchOptions {
  ...
  /**
   * When true (and QUERY_EXPANSION_ENABLED=true env var is set), the original query is
   * rephrased by an LLM into multiple variants. All variants are searched and the results
   * are merged via RRF to improve recall. Defaults to false.
   */
...
export async function federatedSearch(opts: FederatedSearchOptions): Promise<FederatedResult[]> {
  const {
    ...
    expandQueries: shouldExpandQueries = false,
  } = opts;
  ...
  // Determine all query variants to search with
  const queryExpansionEnabled = process.env.QUERY_EXPANSION_ENABLED === "true";
  let queryVariants: string[];

  if (shouldExpandQueries && queryExpansionEnabled) {
    queryVariants = await expandQuery(query);
    logger.debug(
      { original: query.substring(0, 80), variantCount: queryVariants.length },
      "Federated search: query expansion applied"
    );
  } else {
    queryVariants = [query];
  }

  // Cap scoreMap to prevent unbounded memory growth across all variant results
  const MAX_SCORE_MAP = 10_000;
  const scoreMap = new Map<string, { result: FederatedResult; rrfScore: number }>();

  // Run searches for each query variant and merge with RRF
  for (const variant of queryVariants) {
    // Embed query once, reuse across all indexes for this variant
    ...
    if (indexes.includes("kb")) {
      searches.push(
        withTimeout("kb",
          hybridSearch(userId, variant, kbId || null, perIndexLimit).then((chunks) => ({
            source: "kb",
    ...
    const variantResults = await Promise.all(searches);

    // Merge this variant's results into the shared scoreMap via RRF
