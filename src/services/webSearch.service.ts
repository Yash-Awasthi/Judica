...
function resolveProvider(preferred?: SearchProvider): ProviderImpl | null {
  if (preferred) {
    ...
    // Sanitize provider name before logging to prevent log injection
    const safePreferred = preferred.replace(/[\r\n\t]/g, " ").slice(0, 50);
    logger.warn(`Preferred search provider '${safePreferred}' not available, falling back`);
  }
