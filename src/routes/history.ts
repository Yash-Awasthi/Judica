...
const historyPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/search", {
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          after: { type: 'string', description: 'ISO date filter (after)' },
          before: { type: 'string', description: 'ISO date filter (before)' },
        },
      },
      response: {
    ...
    schema: {
      ...
      response: {
        200: { type: 'object', properties: { success: { type: 'boolean' }, title: { type: 'string' } } },
      },
    },
    preHandler: [fastifyRequireAuth, fastifyValidate(renameConversationSchema)],
  }, async (request, _reply) => {
    const { id } = request.params as { id: string };
    ...
    schema: {
      ...
      body: {
        type: 'object',
        required: ['toChatId'],
        properties: { toChatId: { type: 'number', description: 'Fork up to (inclusive) this chat ID' } },
      },
      response: {
        200: {
          type: 'object',
    ...
    preHandler: [fastifyRequireAuth, fastifyValidate(forkSchema)],
  }, async (request, _reply) => {
    const { id } = request.params as { id: string };
