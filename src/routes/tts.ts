...
async function piperTts(text: string, voice?: string): Promise<ArrayBuffer> {
  const piperUrl = (env as Record<string, string>).PIPER_TTS_URL;
...
async function edgeTts(text: string, voice?: string): Promise<ArrayBuffer> {
  const edgeUrl = (env as Record<string, string>).EDGE_TTS_URL;
...
const ttsPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/providers", { preHandler: fastifyRequireAuth }, async (_req, reply) => {
    return reply.send({
      providers: [
        { id: "piper",     label: "Piper TTS (local)",       tier: "free",  configured: Boolean((env as Record<string, string>).PIPER_TTS_URL), url: "https://github.com/rhasspy/piper" },
        { id: "edge-tts",  label: "edge-tts (Microsoft Edge)", tier: "free", configured: Boolean((env as Record<string, string>).EDGE_TTS_URL), url: "https://github.com/rany2/edge-tts" },
        { id: "siliconflow", label: "SiliconFlow / MiMo TTS", tier: "paid",  configured: Boolean((env as Record<string, string>).XIAOMI_MIMO_API_KEY), warning: "Uses SiliconFlow credits" },
  ...
  fastify.post("/", { preHandler: fastifyRequireAuth }, async (request, reply) => {
    ...
    try {
      ...
      if ((env as Record<string, string>).PIPER_TTS_URL) {
    ...
    try {
      ...
      if ((env as Record<string, string>).EDGE_TTS_URL) {
        logger.warn({ err: (e2 as Error).message }, "edge-tts failed, trying paid providers");
      }
    }

    // ── 3. Paid providers (require API key) ──────────────────────────────────
    const API_KEY = (env as Record<string, string>).XIAOMI_MIMO_API_KEY || env.OPENAI_API_KEY;
    if (!API_KEY) {
      reply.code(503);
      return {
    ...
    try {
      ...
      logger.warn({ err: (e3 as Error).message }, "TTS Attempt 3 (xiaomi/MiMo-TTS-v2) failed");
      try {
        audioBuffer = await attemptTTS("https://api.siliconflow.cn/v1/audio/speech", {
          model: "FunAudioLLM/CosyVoice2-0.5B", input: parsed.data.input, voice: "alex"
        });
      } catch (e4: unknown) {
        logger.warn({ err: (e4 as Error).message }, "TTS Attempt 4 (CosyVoice) failed");
