...
/** Escape characters that are significant in HTML attribute values */
...
export default async function widgetPlugin(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get("/snippet", {
    ...
    const baseUrl = escAttr(q.baseUrl || "{YOUR_SERVER_URL}");
    const apiKey = escAttr(q.apiKey || "{YOUR_API_KEY}");
