...
const authPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  const authRateLimit = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // MFA check — if enabled, return a challenge instead of issuing a session
    const mfaRequired = await isMFARequired(user.id);
    if (mfaRequired) {
      logger.info({ username }, "MFA required, returning challenge");
      return reply.code(200).send({ success: false, mfaRequired: true, userId: user.id });
    }
