...
const councilPlugin: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get("/summons", {
    schema: {
      summary: 'List available council summon presets',
      tags: ['Council'],
      response: {
        200: { type: 'object', properties: { summons: { type: 'object' } } },
      },
    },
  }, async (_request, _reply) => {
    ...
    schema: {
      summary: 'List council composition templates',
      tags: ['Council'],
      response: {
        200: { type: 'object', properties: { templates: { type: 'object' } } },
      },
    },
  }, async (_request, _reply) => {
    ...
    fastify.get("/config", {
    schema: {
      summary: 'Get user council configuration',
      tags: ['Council'],
      response: {
        200: { type: 'object', properties: { config: { nullable: true } } },
      },
    },
    ...
    fastify.put("/config", {
    schema: {
      summary: 'Update user council configuration',
      tags: ['Council'],
      body: {
        type: 'object',
        properties: {
          ...
          defaultSummon: { type: 'string' },
          defaultRounds: { type: 'number', minimum: 1, maximum: 5 },
        },
      },
      response: {
    ...
    preHandler: [fastifyRequireAuth, fastifyValidate(updateConfigSchema)],
  }, async (request, _reply) => {
    try {
      const userId = request.userId!;
    ...
    schema: {
      summary: 'Delete user council configuration',
      tags: ['Council'],
      response: {
    ...
    fastify.get("/archetypes/:id", {
    schema: {
      summary: 'Get a single archetype by ID',
      tags: ['Council'],
      params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
      response: {
        200: { type: 'object', properties: { archetype: { type: 'object' } } },
