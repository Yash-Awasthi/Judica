...
import { isLiteMode } from "../lib/deploymentMode.js";
import { startCleanupWorker, scheduleCleanupJob, cleanupQueue } from "./workers/cleanup.worker.js";
...
export function startWorkers() {
  ...
  // Start document TTL cleanup worker
  cleanupWorker = startCleanupWorker();
  // Schedule the repeatable hourly cleanup job (fire-and-forget)
  scheduleCleanupJob().catch((err) => logger.warn({ err }, "Failed to schedule cleanup job"));

  const workers = [ingestionWorker, repoWorker, researchWorker, compactionWorker, cleanupWorker].filter(Boolean);
  ...
  logger.info({ mode: isLiteMode() ? "lite" : "standard" }, "BullMQ workers started (ingestion, repo, research, compaction, cleanup)");
}
