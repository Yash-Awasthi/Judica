...
import { verifyDiscordSignature, handleInteraction, handleMessageCreate } from "./events.js";
import type { DiscordMessageCreateEvent } from "./events.js";
...
const discordPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  // ─── POST /messages — Discord MESSAGE_CREATE gateway events ─────────
  // Receives forwarded MESSAGE_CREATE events from a gateway proxy.
  // Signature verification uses the same Ed25519 public key as /interactions.
  fastify.post("/messages", { config: { rateLimit: { max: 120, timeWindow: "1 minute" } } }, async (request, reply) => {
    const rawBody = JSON.stringify(request.body);
    const signature = request.headers["x-signature-ed25519"] as string;
    const timestamp = request.headers["x-signature-timestamp"] as string;
    ...
    const { bot_user_id, message } = request.body as {
      bot_user_id?: string;
      message?: DiscordMessageCreateEvent;
    };

    if (!message || !bot_user_id) {
      reply.code(400);
      return { error: "Missing bot_user_id or message in payload" };
    }
    ...
    handleMessageCreate(message, bot_user_id, config).catch((err) => {
      log.error({ err }, "Async Discord MESSAGE_CREATE handling failed");
    });
