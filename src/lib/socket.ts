  ...
  wss.on("connection", (ws: ClientSocket) => {
    ...
    ws.on("message", async (raw) => {
      ...
      try {
        ...
        if (Buffer.byteLength(raw as Buffer) > 16 * 1024) return;
        ...
        if (msg.type === "join:conversation" && msg.conversationId) {
        ...
        } else if (msg.type === "user:typing" && msg.conversationId) {
          ...
          if (ws.rooms!.has(room)) {
            const payload = JSON.stringify({
              event: "user:typing",
              data: { userId: ws.userId, username: ws.username, conversationId: msg.conversationId },
            });
            for (const client of wss!.clients as Set<ClientSocket>) {
              if (client !== ws && client.readyState === WebSocket.OPEN && client.rooms?.has(room)) {
                client.send(payload);
              }
            }
          }
