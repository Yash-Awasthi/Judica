...
import { hooks } from "./hooks/hookRegistry.js";
...
export async function* deliberate(
  ...
  for (let r = 1; r <= rounds; r++) {
    ...
    // Hook: pre:llm — before member LLM calls
    await hooks.run('pre:llm', { stage: 'pre:llm', query: currentMessages[currentMessages.length - 1]?.content as string | undefined });
    ...
    // Hook: post:llm — after member LLM calls
    await hooks.run('post:llm', { stage: 'post:llm', response: opinions.map(o => o.opinion).join('\n---\n') });
  ...
  // Hook: pre:llm — before master synthesis LLM call
  await hooks.run('pre:llm', { stage: 'pre:llm', query: currentMessages[currentMessages.length - 1]?.content as string | undefined });
  ...
  // Hook: post:llm — after master synthesis LLM call
