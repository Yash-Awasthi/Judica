...
function modelSupportsCaching(model: string | undefined): boolean {
  if (!model) return false;
  const m = model.toLowerCase();
  return CACHE_SUPPORTED_MODELS.some((prefix) => m.includes(prefix));
}

function isCacheEnabled(): boolean {
  return process.env.ANTHROPIC_PROMPT_CACHE === "true";
}
...
export class AnthropicProvider extends BaseProvider {
  ...
  async call({ messages, signal, maxTokens, isFallback, onChunk, _depth = 0 }: {
    ...
    try {
      const useCache = isCacheEnabled() && modelSupportsCaching(this.config.model);

      // Build system field: if caching is enabled and system prompt is large enough,
      // use a content-block array with cache_control instead of a plain string.
      let systemField: unknown = this.config.systemPrompt
        ? this.config.systemPrompt
        : undefined;

      if (useCache && this.config.systemPrompt && this.config.systemPrompt.length > 1024) {
        systemField = [
          {
            type: "text",
      ...
      // Build message list; add cache_control to document-context blocks when caching is enabled.
      const builtMessages = messages.map((m) => {
        const role = m.role === "system" ? "user" : (m.role === "tool" ? "user" : m.role);
        let content = m.content;

        if (useCache && typeof content === "string" && content.length > 0) {
          // Heuristic: document-context blocks start with a known tag or are injected knowledge
          const isDocContext =
            content.startsWith("[KNOWLEDGE BASE]") ||
            content.startsWith("[CODE REPOSITORY]") ||
            content.startsWith("[CONVERSATION HISTORY]") ||
            content.startsWith("[COUNCIL FACTS]");

          if (isDocContext) {
            // cache_control is an Anthropic API extension not in the shared ContentBlock type.
            // We cast through unknown so the JSON body is correct while keeping TypeScript happy.
            content = [
              {
                type: "text" as const,
                text: content,
                cache_control: { type: "ephemeral" },
              } as unknown as import("../types.js").ContentBlock,
            ];
          }
        }
            ...
            if (line.startsWith("data: ")) {
              ...
              try {
                ...
                if (parsed.type === "message_start" && parsed.message?.usage) {
                  ...
                  // Log streaming cache hit
                  if ((parsed.message.usage.cache_read_input_tokens ?? 0) > 0) {
                    logger.info(
                      {
                        provider: this.name,
                      ...
                      "Anthropic prompt cache hit (streaming)"
                    );
                  }
                }
      ...
      // Log cache hit when the response reports reused cached tokens
      if ((data.usage?.cache_read_input_tokens ?? 0) > 0) {
        logger.info(
          {
            provider: this.name,
            model: this.config.model,
