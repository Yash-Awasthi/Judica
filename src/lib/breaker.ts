...
// TTL-based eviction — breakers unused for 10 minutes are cleaned up
...
// Periodic cleanup of stale breakers (every 5 minutes)
setInterval(() => {
  const now = Date.now();
...
const BREAKER_OPTIONS = {
  ...
  // Only count 5xx server errors toward the failure threshold.
...
export function getBreaker<T extends (...args: unknown[]) => Promise<unknown>>(
  ...
  // Disable Opossum's built-in timeout — we use external AbortController.
...
// Runtime guard on fire() return value.
...
function wrapBreaker(breaker: CircuitBreaker, provider: Pick<Provider, "name">): CircuitBreaker {
  ...
  breaker.fire = async function (...args: unknown[]) {
    ...
    // Runtime type guard — reject non-Response values
