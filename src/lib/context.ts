...
// Enriched request context — carries userId, traceId, tenantId alongside requestId
// so downstream code doesn't need to re-fetch from the request object.
export interface RequestContext {
  readonly requestId: string;
  // OTEL trace/span correlation — injected by tracing middleware
  readonly traceId?: string;
  readonly spanId?: string;
  // User and tenant context for downstream access
...
// Helper for context-wrapped execution — eliminates manual store.run() boilerplate
export function withContext<T>(ctx: RequestContext, fn: () => T): T {
  return requestContext.run(ctx, fn);
}

// DEPRECATED — withContext already handles async functions correctly.
...
// Context is now immutable via `readonly` properties on the interface.
