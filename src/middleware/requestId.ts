...
// Validate request ID format — accept alphanumeric strings with hyphens
...
export async function fastifyRequestId(request: FastifyRequest, reply: FastifyReply) {
  ...
  // Only trust header value if it matches a safe format
  const id = (header && REQUEST_ID_RE.test(header)) ? header : randomUUID();
  (request as unknown as { requestId: string }).requestId = id;
  reply.header("X-Request-ID", id);
  // Initialize AsyncLocalStorage context so logger mixin and getContextOrThrow() work
