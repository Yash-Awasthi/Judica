...
export async function fastifyCspNonce(request: FastifyRequest, reply: FastifyReply) {
  // Only apply CSP to HTML responses — skip for API (JSON) responses
  ...
  // Derive allowed WS origin from FRONTEND_URL or request host
  let wsOrigin: string;
  ...
  reply.header(
    ...
    [
      "default-src 'self'",
      // Pin cdn.jsdelivr.net to specific package subpaths
      `script-src 'self' 'nonce-${nonce}' https://cdn.jsdelivr.net/npm/`,
      // Removed unsafe-inline — use nonce-based inline styles instead
      `style-src 'self' 'nonce-${nonce}' https://cdn.jsdelivr.net/npm/`,
      "img-src 'self' data: blob:",
      // Restrict WebSocket connections to our domain only
      ...
      // Report CSP violations for detection
