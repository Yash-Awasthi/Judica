...
// Redis client for @fastify/rate-limit — properly tracks connection state
...
export function getRateLimitRedis(): InstanceType<typeof IORedis> | undefined {
  // Only return client if it's actually connected and ready
  return redisReady ? rateLimitRedisClient : undefined;
}

/** Check if rate-limit Redis is healthy */
...
export async function cleanupRateLimitRedis(): Promise<void> {
  if (rateLimitRedisClient) {
    try {
      // Timeout quit to prevent blocking shutdown if Redis is hung
      await Promise.race([
